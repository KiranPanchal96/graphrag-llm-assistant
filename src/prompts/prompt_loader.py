...
def load_prompt_from_langfuse(
    langfuse: Langfuse, prompt_name: str, fallback: str
) -> str:
    try:
        ...
        print(
            f"Could not fetch '{prompt_name}' prompt from Langfuse, using static default.",
            e,
        )
        return fallback
...
def get_suffix_prompt(langfuse: Langfuse) -> str:
    if PROMPT_SOURCE == "manual":
        ...
        return load_prompt_from_langfuse(langfuse, "main_rag_suffix", SUFFIX_PROMPT)
