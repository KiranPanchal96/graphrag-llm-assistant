...
def evaluate_model(data: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for item in data:
        ...
        print(f"\n❓ Q: {question}")
        output: dict[str, Any] = run_rag_pipeline(question)
        prediction = str(output["answer"])
        sources: list[Document] = output.get("sources", [])
