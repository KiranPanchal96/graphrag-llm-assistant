...
# Endpoint Config
# ---------------------------
API_HOST: str = os.getenv("FASTAPI_HOST", "localhost")
API_PORT: str = os.getenv("FASTAPI_PORT", "8000")
API_HEALTH_URL: str = f"http://{API_HOST}:{API_PORT}/"
API_RAG_URL: str = f"http://{API_HOST}:{API_PORT}/rag"
...
def check_fastapi() -> bool:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(
                f"⚠️  FastAPI responded with status {resp.status_code} at {API_HEALTH_URL}"
            )
            return False
    except Exception as e:
        print(
            f"❌ FastAPI not running at {API_HEALTH_URL}. Please start your FastAPI server!\nError: {e}"
        )
        sys.exit(1)
...
def query_rag(question: str) -> tuple[str, str]:
    """Send a query to the FastAPI RAG endpoint."""
...
with gr.Blocks(title="Life Strategy RAG UI") as demo:
    ...
    # Bind events
    ask_button.click(
        fn=query_rag, inputs=question_input, outputs=[answer_output, sources_output]
    )
