...
def test_langfuse_trace_created():
    ...
    assert resp.status_code == 200, f"Langfuse API returned {resp.status_code}: {resp.text}"
    ...
    found = any(
        prompt[:10].lower() in (str(trace.get("name", "")).lower() + str(trace.get("input", "")).lower())
        for trace in traces
    )
    assert found, f"Prompt snippet '{prompt[:10]}' not found in recent Langfuse traces: {str(traces)}"

if __name__ == "__main__":
    # For manual command-line runs, see nice pytest output
    import sys
